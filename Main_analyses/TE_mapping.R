...
system(paste0("mkdir -p Outputs/",folder,"/", data, "/", condition, "/", res_dir, "/"))
out_dir <- paste0("Outputs/",folder,"/", data, "/", condition, "/", res_dir, "/")
...
if(data_name=="RNAseq"){
  reads_whole = read.table(paste0("Inputs/counts/",data_name, "_batch.age.corrected.txt"), header=TRUE, row.names=1)
}else{
  reads_whole = read.table(paste0("Outputs/corrected_reads/",data_name, "_batch.age.corrected.txt"), header=TRUE, row.names=1)
}

## Load positions file
genepos = read.table(paste0("Inputs/positions/",data, "_positions.txt"),header = TRUE, stringsAsFactors = FALSE)
## Load SNP positions
snpspos = read.table(paste0("Inputs/TE_genotypes/TE_positions.txt"),header = TRUE, stringsAsFactors = FALSE)
## Load genotype info for SNPs
gtypes = read.table(paste0("Inputs/TE_genotypes/TE_genotypes.txt"),header = TRUE, stringsAsFactors = FALSE)
...
## Save files.
snps_positions_file_name="Inputs/TE_genotypes/TE_positions.txt"
gene_positions_file_name=paste0("Inputs/TE_genotypes/", data, "_positions.txt")
